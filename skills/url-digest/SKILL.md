...
/Users/kta/.local/share/life/_life/daily/YYYY-MM-DD-digest.html    (non-git, served at /daily/)
```
...
9. Render `/Users/kta/.local/share/life/_life/daily/YYYY-MM-DD-digest.html` from the Markdown source.
...
```bash
python3 <skill-dir>/scripts/render_digest_html.py ideas/daily/md/YYYY-MM-DD-digest.md -o /Users/kta/.local/share/life/_life/daily/YYYY-MM-DD-digest.html
```

**IMPORTANT — `/Users/kta/.local/share/life/_life/` write rule (life#77 follow-up):**

claude binary は LaunchDaemon context で macOS TCC により `/Users/kta/.local/share/life/_life/` への直接書き込みが拒否される。

- **NEVER use the Write tool to write a file under `/Users/kta/.local/share/life/_life/...` directly.**
- `/Users/kta/.local/share/life/_life/` への書き込みは **必ず `python3 <render-script>` を bash 経由で呼び出す** (python3 は FDA grant 済)。本 skill では `render_digest_html.py` と `refresh_daily_index.py` がそれ。
...
Replace `YYYY-MM-DD` with the digest day. Only the digest Markdown is committed — the HTML and index.json live under `/Users/kta/.local/share/life/_life/daily/` (non-git, #76 Phase 3).
