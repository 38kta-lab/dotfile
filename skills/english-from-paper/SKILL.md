...
portfolio/paper-close-readings/
  ...
  tmp/<slug>/clean-original.html  ← English cleaned HTML (HTML mode) — required for NotebookLM upload
  tmp/<slug>/Fig.*.png            ← figure assets
```

Note the **per-slug tmp subdir** convention from paper-close-reading: every `tmp/...` path below means `tmp/<slug>/...`.
...
Fixed 20 questions total (8 cloze / 5 collocation / 7 comprehension — see Quality Bars), mixed format:
...
sources being `tmp/<slug>/clean-original.html` + `<slug>-en-study.md`.
...
mkdir -p "$DST"
cp portfolio/paper-close-readings/tmp/<slug>/clean-original.html  "$DST/"
python3 scripts/papers/html_to_txt.py \
  portfolio/paper-close-readings/tmp/<slug>/clean-original.html \
...
`scripts/automation/regen_paper_index.py` must skip the english-study sibling files so they don't appear as spurious "paper" rows. **This patch is already applied** in the `life` repo (the SKIP set below). If a fresh checkout lacks it, re-apply before running render_all:
...
2. **Read** `tmp/<slug>/clean-original.html` (HTML mode) or text-extracted PDF (PDF mode) for the canonical English source.
