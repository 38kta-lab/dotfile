#!/usr/bin/env python3
"""Render a weekly-review Markdown file to standalone HTML."""
...
CSS_PATH = SKILL_DIR / "assets" / "newsprint-weekly.css"
...
def inline_md(text: str) -> str:
    ...
    escaped = re.sub(
        r"&lt;(https?://[^&]+)&gt;",
        lambda m: stash(f'<a href="{m.group(1)}">{m.group(1)}</a>'),
        escaped,
    )
...
def parse_blocks(lines: list[str]) -> str:
    ...
    while i < len(lines):
        ...
        if re.match(r"^\s*[-*]\s+", line):
            ...
            item = re.sub(r"^\[( |x|X)\]\s+", lambda m: "☑ " if m.group(1).lower() == "x" else "☐ ", item)
